...
import { Viewport }        from './ui/viewport.js?v=6';
import { PropertiesPanel } from './ui/properties.js?v=6';
import { MenuBar }         from './ui/menu.js?v=6';
import { renderMarkdown }  from './ui/markdown.js?v=6';
import { UndoStack }       from './utils/undo.js?v=6';
import { esc }             from './utils/escape.js?v=6';
import { StaticSolver, ensureDefaultLC }   from './solver/static_solver.js?v=6';
import { Results }                         from './solver/postprocess.js?v=6';
import { assessStabilitySanity, STABILITY } from './solver/stability.js?v=6';
import { areaStress, areaBendingStress, vonMises, areaLocalFrame } from './solver/membrane.js?v=6';
import { ModalSolver, guyanReduce }        from './solver/modal_solver.js?v=6';
import { buildNodeIndex, assembleK, assembleF, getNodeDOFs } from './solver/assembler.js?v=6';
import { assembleSparseGlobal, extractFreeCSR } from './solver/sparse.js?v=6';
import { solveCorotBeam } from './solver/corotbeam.js?v=6';
import { insertInfill } from './model/macromodel.js?v=6';
import { listMacros, getMacro, insertMacro } from './model/macro_registry.js?v=6';
import { assembleKg } from './solver/geometric.js?v=6';
import { makeFactor } from './solver/linsolve.js?v=6';
import { ModalResults }                    from './solver/modal_results.js?v=6';
import { parseAccelerogram, accStats, scaleToPGA, DEMO_PRESETS, G as GACC } from './solver/accelerograms.js?v=6';
import { tendonEquivalentLoads, applyTendon, tendonEcc } from './solver/tendon.js?v=6';
import { computeMovingLoad } from './solver/moving_load.js?v=6';
import { buildShearStories, runShearHistory, shearFreqs } from './solver/shear_building.js?v=6';
import { solvePlastic } from './solver/plastic.js?v=6';
import { linearBuckling, pDelta } from './solver/geometric_analysis.js?v=6';
import { buildNLTrussProblem, buildCorotProblem, remapCorotSteps, buildFormFindProblem, setupPushoverControl } from './solver/nl_frame.js?v=6';
import { driftLimit } from './design/serviceability.js?v=6';
import { buildSpectrum } from './design/nch433_spectrum.js?v=6';
import { selectProfile, steelCandidates, predimensionar, candidatesForFamily } from './design/autodesign.js?v=6';
import { jointSCWB } from './design/seismic.js?v=6';
...
import { smoothAreasInModel } from './model/mesh_quality.js?v=6';
import { listFormats, exportModel, importModel } from './io/index.js?v=6';
import { extensions } from './ext/extensions.js?v=6';
import { loadBranding, getBranding } from './branding.js?v=6';
import { SpectrumSolver }    from './solver/spectrum_solver.js?v=6';
import { modalTimeHistory }  from './solver/timehistory.js?v=6';
...
import { reportHTML, reportDocx, reportDocxCover, reportDocxBody } from './report/report.js?v=6';
...
class App {
  ...
  _staticWorkerSolve(K, nDOF, freeDOF, Flist, dense = false) {
    return new Promise((resolve, reject) => {
      let worker;
      try { worker = new Worker(new URL('./solver/static_worker.js?v=6', import.meta.url), { type: 'module' }); }
  ...
  async runModal(opts = {}) {
    ...
    try {
      ...
      let modes = await new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./solver/modal_worker.js?v=6', import.meta.url), { type: 'module' });
  ...
  async runTimeHistory(opts = {}) {
    ...
    try {
      ...
      const rawModes = await new Promise((resolve, reject) => {
        const w = new Worker(new URL('./solver/modal_worker.js?v=6', import.meta.url), { type: 'module' });
  ...
  _thSolveInWorker(modes, ag, dt, zeta) {
    return new Promise((resolve, reject) => {
      let w;
      try { w = new Worker(new URL('./solver/timehistory_worker.js?v=6', import.meta.url), { type: 'module' }); }
      catch (e) {
...
4,100</textarea></div>
  ...
  async predimensionarDialog() {
    ...
    if (document.getElementById('pd-assign').checked) {
      const { profileToSection } = await import('./design/profiles.js?v=6');
      this.snapshot();
  ...
  _solveNLInWorker(kind, opts) {
    return new Promise((resolve, reject) => {
      let worker;
      try {
        worker = new Worker(new URL('./solver/nl_worker.js?v=6', import.meta.url), { type: 'module' });
      } catch (e) {
...
<details style="margin-top:12px">
  ...
  async applyModelOperations(ops, ctx = null) {
    ...
    this.snapshot();
    const { applyOperations } = await import('./model/model_ops.js?v=6');
  ...
  async _generateFromSpec(specText) {
    ...
    try {
      ...
      const { generateModel } = await import('../assistant/generator.js?v=6');
    ...
    inp.onchange = async () => {
      ...
      try {
        mods = await Promise.all([
          import('./io/ifc/ifcToPortico.js?v=6'),
          import('./ui/ifcImportDialog.js?v=6'),
          import('./io/neutral.js?v=6'),
        ]);
  ...
  async exportIFC() {
    ...
    try {
      const [{ modelToNeutral }, { neutralToIFC }] = await Promise.all([
        import('./io/neutral.js?v=6'),
        import('./io/ifc/ifcWriter.js?v=6'),
      ]);
      const m = this._predisc ? this.serializer.fromJSON(this._predisc) : this.model;
  ...
  async generateReportDocx() {
    ...
    try {
      const { Docx } = await import('./io/docx.js?v=6');
      let d;
      if (multi) {
