...
import { Viewport }        from './ui/viewport.js?v=7';
import { PropertiesPanel } from './ui/properties.js?v=7';
import { MenuBar }         from './ui/menu.js?v=7';
import { renderMarkdown }  from './ui/markdown.js?v=7';
import { UndoStack }       from './utils/undo.js?v=7';
import { esc }             from './utils/escape.js?v=7';
import { StaticSolver, ensureDefaultLC }   from './solver/static_solver.js?v=7';
import { Results }                         from './solver/postprocess.js?v=7';
import { assessStabilitySanity, STABILITY } from './solver/stability.js?v=7';
import { areaStress, areaBendingStress, vonMises, areaLocalFrame } from './solver/membrane.js?v=7';
import { ModalSolver, guyanReduce }        from './solver/modal_solver.js?v=7';
import { buildNodeIndex, assembleK, assembleF, getNodeDOFs } from './solver/assembler.js?v=7';
import { assembleSparseGlobal, extractFreeCSR } from './solver/sparse.js?v=7';
import { solveCorotBeam } from './solver/corotbeam.js?v=7';
import { insertInfill } from './model/macromodel.js?v=7';
import { listMacros, getMacro, insertMacro } from './model/macro_registry.js?v=7';
import { assembleKg } from './solver/geometric.js?v=7';
import { makeFactor } from './solver/linsolve.js?v=7';
import { ModalResults }                    from './solver/modal_results.js?v=7';
import { parseAccelerogram, accStats, scaleToPGA, DEMO_PRESETS, G as GACC } from './solver/accelerograms.js?v=7';
import { tendonEquivalentLoads, applyTendon, tendonEcc } from './solver/tendon.js?v=7';
import { computeMovingLoad } from './solver/moving_load.js?v=7';
import { buildShearStories, runShearHistory, shearFreqs } from './solver/shear_building.js?v=7';
import { solvePlastic } from './solver/plastic.js?v=7';
import { linearBuckling, pDelta } from './solver/geometric_analysis.js?v=7';
import { buildNLTrussProblem, buildCorotProblem, remapCorotSteps, buildFormFindProblem, setupPushoverControl } from './solver/nl_frame.js?v=7';
import { driftLimit } from './design/serviceability.js?v=7';
import { buildSpectrum } from './design/nch433_spectrum.js?v=7';
import { selectProfile, steelCandidates, predimensionar, candidatesForFamily } from './design/autodesign.js?v=7';
import { jointSCWB } from './design/seismic.js?v=7';
import { resolveMaterial } from './design/material_props.js?v=7';
import { resolveSectionProps } from './design/section_props.js?v=7';
import { autoDetectDiaphragms, computeFloorCR } from './solver/diaphragm.js?v=7';
import { interstoryDrifts, buildStoryLevels } from './solver/drift.js?v=7';
import { splitElement, splitByLength, discretizeAll, joinElements, intersectElements } from './model/discretize.js?v=7';
import { localAxes, transformMatrix, globalStiffness, applyReleases, elemLocalK, elemLocalM, rigidEndOffsets } from './solver/timoshenko.js?v=7';
import { blockCells, cornerGridIndices } from './model/mesher.js?v=7';
import { coonsGridFromCorners } from './model/mesh_map.js?v=7';
import { meshPolygonIntoModel } from './model/mesh_free.js?v=7';
import { smoothAreasInModel } from './model/mesh_quality.js?v=7';
...
import { reportHTML, reportDocx, reportDocxCover, reportDocxBody } from './report/report.js?v=7';
...
class App {
  ...
  _initHelp() {
    ...
    // Documentation viewer: back button + delegated internal-link (data-doc) navigation.
    document.getElementById('docs-back')?.addEventListener('click', () => this._docsShowIndex());
    document.getElementById('docs-body')?.addEventListener('click', e => {
      const a = e.target.closest('a[data-doc]');
      if (!a) return;
      e.preventDefault();
      this._openDoc(a.dataset.doc);
    });
    // Re-render the open doc / the index in the new language when the locale changes.
    window.addEventListener('localechange', () => {
      if (document.getElementById('help-overlay')?.classList.contains('hidden')) return;
      if (this._curDoc) this._openDoc(this._localizeDocPath(this._curDoc));
      else this._renderDocsIndex();
    });
  }

  // Catalogue of in-app documents (canonical .md path; the .es.md variant is derived).
  get _docCatalog() {
    return [
      { group: 'Manuales', groupEn: 'Manuals', items: [
        { md: 'docs/analysis-reference.md', es: 'Manual de Análisis (teoría)', en: 'Analysis Reference (theory)',
          descEs: 'Fundamentos del motor: elementos, ensamblaje, dinámica, no-linealidad, diseño.',
          descEn: 'Engine fundamentals: elements, assembly, dynamics, nonlinearity, design.' },
        { md: 'docs/verification-manual.md', es: 'Manual de Verificación (validaciones)', en: 'Verification Manual (validations)',
          descEs: 'Casos contrastados contra solución analítica / SAP2000 / equilibrio global.',
          descEn: 'Cases checked against analytical / SAP2000 / global-equilibrium solutions.' },
      ] },
      { group: 'Tutoriales', groupEn: 'Tutorials', items: [
        { md: 'docs/tutorials/01-valdivia-nch433.md', es: 'Tutorial 1 — Edificio en Valdivia (NCh433)', en: 'Tutorial 1 — Valdivia building (NCh433)',
          descEs: 'Hormigón armado, caja de escalera shell, losas placa, diseño y derivas.',
          descEn: 'Reinforced concrete, shell stair core, plate slabs, design and drifts.' },
        { md: 'docs/tutorials/02-pushover-collapse.md', es: 'Tutorial 2 — Pushover a colapso', en: 'Tutorial 2 — Pushover to collapse',
          descEs: 'Pórtico de acero: rótulas plásticas evento-a-evento hasta el mecanismo.',
          descEn: 'Steel frame: event-to-event plastic hinges to the collapse mechanism.' },
        { md: 'docs/tutorials/03-performance-based.md', es: 'Tutorial 3 — Evaluación por desempeño', en: 'Tutorial 3 — Performance-based assessment',
          descEs: 'Método de los coeficientes ASCE 41 sobre la curva de capacidad.',
          descEn: 'ASCE 41 coefficient method over the capacity curve.' },
      ] },
    ];
  }

  // Map a canonical .md path to the locale variant ('foo.md' → 'foo.es.md' in Spanish).
  _localizeDocPath(md) {
    return i18n.getLocale() === 'en' ? md : md.replace(/\.md$/, '.es.md');
  }
  ...
  _renderDocsIndex() {
    ...
    box.innerHTML = this._docCatalog.map(sec => `
      <h3 class="doc-group">${en ? sec.groupEn : sec.group}</h3>
      <div class="doc-cards">${sec.items.map(it => `
        <button class="doc-card" data-doc="${this._localizeDocPath(it.md)}">
          <span class="doc-card-t">${en ? it.en : it.es}</span>
          <span class="doc-card-d">${en ? it.descEn : it.descEs}</span>
        </button>`).join('')}</div>`).join('');
    box.querySelectorAll('.doc-card').forEach(c =>
      c.addEventListener('click', () => this._openDoc(c.dataset.doc)));
  }

  _docsShowIndex() {
    this._curDoc = null;
    document.getElementById('docs-view')?.classList.add('hidden');
    document.getElementById('docs-index')?.classList.remove('hidden');
  }

  // Fetch a doc (by locale-specific path) and render it in place. Stores the canonical
  // .md path so a later locale change can reload the sibling-language file.
  async _openDoc(path) {
    const body = document.getElementById('docs-body');
    if (!body) return;
    this._curDoc = path.replace(/\.es\.md$/, '.md');
    document.getElementById('docs-index')?.classList.add('hidden');
    document.getElementById('docs-view')?.classList.remove('hidden');
    body.innerHTML = `<p class="help-note">${i18n.t('Cargando…')}</p>`;
    body.scrollTop = 0;
    const base = path.replace(/[^/]*$/, '');   // directory of the doc
    try {
      const r = await fetch(path, { cache: 'no-store' });
      if (!r.ok) throw new Error(String(r.status));
      body.innerHTML = renderMarkdown(await r.text(), base);
    } catch (e) {
      body.innerHTML = `<p class="help-note">${i18n.t('No se pudo cargar el documento')}: <code>${path}</code></p>`;
    }
  ...
  _switchHelpSec(sec) {
    ...
    if (sec === 'docs' && !document.getElementById('docs-list')?.children.length) this._renderDocsIndex();
  }
  ...
  openHelp(sec = 'guia') {
    ...
    if (sec === 'docs') { this._docsShowIndex(); this._renderDocsIndex(); }
  }
  ...
  _staticWorkerSolve(K, nDOF, freeDOF, Flist, dense = false) {
    return new Promise((resolve, reject) => {
      let worker;
      try { worker = new Worker(new URL('./solver/static_worker.js?v=7', import.meta.url), { type: 'module' }); }
  ...
  async runModal(opts = {}) {
    ...
    try {
      ...
      let modes = await new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./solver/modal_worker.js?v=7', import.meta.url), { type: 'module' });
  ...
  async runTimeHistory(opts = {}) {
    ...
    try {
      ...
      const rawModes = await new Promise((resolve, reject) => {
        const w = new Worker(new URL('./solver/modal_worker.js?v=7', import.meta.url), { type: 'module' });
  ...
  _thSolveInWorker(modes, ag, dt, zeta) {
    return new Promise((resolve, reject) => {
      let w;
      try { w = new Worker(new URL('./solver/timehistory_worker.js?v=7', import.meta.url), { type: 'module' }); }
      catch (e) {
...
4,100</textarea></div>
  ...
  async predimensionarDialog() {
    ...
    if (document.getElementById('pd-assign').checked) {
      const { profileToSection } = await import('./design/profiles.js?v=7');
      this.snapshot();
...
<details style="margin-top:12px">
  ...
  async applyModelOperations(ops, ctx = null) {
    ...
    this.snapshot();
    const { applyOperations } = await import('./model/model_ops.js?v=7');
  ...
  async _generateFromSpec(specText) {
    ...
    try {
      ...
      const { generateModel } = await import('../assistant/generator.js?v=7');
    ...
    inp.onchange = async () => {
      ...
      try {
        mods = await Promise.all([
          import('./io/ifc/ifcToPortico.js?v=7'),
          import('./ui/ifcImportDialog.js?v=7'),
          import('./io/neutral.js?v=7'),
        ]);
  ...
  async exportIFC() {
    ...
    try {
      const [{ modelToNeutral }, { neutralToIFC }] = await Promise.all([
        import('./io/neutral.js?v=7'),
        import('./io/ifc/ifcWriter.js?v=7'),
      ]);
      const m = this._predisc ? this.serializer.fromJSON(this._predisc) : this.model;
  ...
  async generateReportDocx() {
    ...
    try {
      const { Docx } = await import('./io/docx.js?v=7');
      let d;
      if (multi) {
