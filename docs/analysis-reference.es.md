...
Este capítulo es la respuesta honesta y escaneable a *"¿qué puede hacer portico-core y qué no?"* — para
que decidas si sirve a tu problema antes de leer la teoría. Cada fila es lo que el código hace de verdad
en **v0.2.0**; los detalles finos están en [§8 Alcance y limitaciones](#8-alcance-y-limitaciones).

**Leyenda:** ✔ disponible · ◐ disponible pero simplificado/parcial (lee la nota) · ✘ no disponible.

### Modelación

| Capacidad | | Notas |
| --- | :-: | --- |
| Elemento de barra — Timoshenko 3D (flexible a corte) | ✔ | Liberaciones, brazos rígidos de extremo, resortes de extremo semirrígidos, fundación elástica (Winkler) |
| Membrana — CST, Quad4, triángulo con giro de Allman | ✔ | Tensión/deformación plana; GDL de giro opcional |
| Flexión de placa — MITC4 (quad), DKT (tri) | ✔ | Placas delgadas y gruesas (Mindlin) |
| Cáscara (membrana + placa combinadas) | ✔ | Compuesta por elemento (`behavior:'shell'`) |
| Reticulado / cable / puntal solo-compresión | ◐ | Solo en la vía no-lineal geométrica (barra de 3 GDL); **sin elemento reticulado lineal, sin flecha/catenaria de cable** |
| Links rígidos, acoplamientos, diafragmas rígidos | ✔ | Formulación por penalización (error de restricción diminuto y acotado) |
| Resortes — elásticos, acoplados/inclinados, unilaterales (despegue), suelo no lineal (p–y/t–z) | ✔ | |
| Elementos sólidos / brick | ✘ | Sin elementos de continuo 3D |
| Secciones — propiedades numéricas o formas paramétricas (I, rect, círculo, CHS, RHS, U, L, T, polígono) | ✔ | Calculadora de propiedades incorporada (S, Z, r, J, Av, Cw) |
| Base de perfiles — europeos IPE / HEA / HEB + CHS / RHS | ✔ | **No incluye tablas americanas W (AISC)** |
| Materiales — acero, hormigón, madera, aluminio (isótropo lineal-elástico) | ✔ | **Sin material ortótropo / no lineal de análisis** |
| Cargas — nodales, distribuidas (uniforme/trapecial; global/local/gravedad/proyectada), térmicas (barra + gradiente de área), peso propio, tendón/pretensado | ✔ | **Sin carga puntual en barra como tipo estático** (agrega un nodo); **sin carga de presión/cara en áreas** |
| Masa — consistente/concentrada, masa puntual nodal + inercia rotacional, fuente de masa estilo ETABS | ✔ | |
| Casos de carga, combinaciones lineales, modos 2D / 3D, CR/CM de diafragma | ✔ | Un solo sistema de unidades (kN, m, t); **sin motor de conversión de unidades** |

### Análisis

| Capacidad | | Notas |
| --- | :-: | --- |
| Estático lineal — casos y combinaciones | ✔ | Combinaciones por superposición lineal |
| Modal / valores propios | ✔ | Iteración inversa (Stodola) con deflación; fuente de masa; participación y masa efectiva |
| Espectro de respuesta — CQC o SRSS | ◐ | **Una dirección horizontal (X o Y) por corrida; sin vertical Z; sin combinación direccional 100/30** |
| Espectro de diseño incorporado — NCh433 / DS61 (Chile) | ✔ | **Sin generador de espectro ASCE 7 / Eurocódigo 8 / IBC** (acepta cualquier curva `[{T,Sa}]`) |
| Pandeo lineal — `(K + λ·Kg)φ = 0`, iteración por subespacios | ✔ | Rigidez geométrica flexural de barra; pandeo de cáscara solo parcial |
| P-Delta (segundo orden) | ✔ | Iteración tangente de punto fijo (no Newton/longitud de arco) |
| No lineal geométrico — gran desplazamiento reticulado/cable, viga corotacional plana, form-finding | ✔ | **La viga corotacional es plana (X–Z); sin viga de gran rotación 3D** |
| Modal pretensado / con rigidez geométrica (precargado) | ✔ | |
| Pushover de rótulas plásticas — evento a evento hasta el colapso | ✔ | Rótulas concentradas (N, V, M); caída dúctil/frágil; colapso = mecanismo |
| Pushover por control de desplazamiento | ◐ | Longitud de arco sobre la idealización axial (reticulado/cable) — snap-through geométrico, no un pushover de rótulas |
| Historia en el tiempo no lineal | ◐ | **Edificio de corte elastoplástico equivalente** (1 GDL/diafragma, pisos bilineales) — no plasticidad distribuida/fibras |
| Historia en el tiempo lineal — superposición modal (recurrencia exacta) | ✔ | Expuesta en la app; no en la API pública |
| Construcción por etapas / secuencial | ✔ | **Solo barras — áreas y diafragmas se ignoran en el etapado**; sin fluencia/retracción |
| Cargas móviles / líneas de influencia | ✔ | Cargas verticales |
| Plasticidad distribuida / secciones de fibra, contacto/gap, dinámica explícita | ✘ | La inelasticidad es rótulas concentradas o resortes de piso 1-D |

### Verificaciones de diseño (demanda/capacidad)

| Capacidad | | Notas |
| --- | :-: | --- |
| Acero — AISC 360-16 (LRFD/ASD) | ◐ | Fluencia, pandeo flexural E3, flexión F2/F6/F9/F10, corte G2, interacción H1. **Sin rotura en sección neta, pandeo torsional/flexo-torsional, elementos esbeltos (E7) ni torsión** |
| Acero — Eurocódigo 3 (EN 1993-1-1) | ◐ | Clase 1–3, curvas de pandeo, LTB, interacción 6.3.3. **Sin ancho efectivo Clase 4, sección neta ni pandeo de alma por corte** |
| Hormigón — ACI 318-19 | ◐ | Flexión, corte, **interacción P–M real (incl. biaxial)**. **Solo secciones rectangulares; sin detallamiento, torsión, esbeltez ni punzonamiento** |
| Hormigón — Eurocódigo 2 (EN 1992-1-1) | ◐ | **Un alias del procedimiento ACI** — no los factores parciales / bloque parábola-rectángulo de EC2 |
| Aluminio — Eurocódigo 9 (EN 1999-1-1) | ◐ | Tracción, compresión, LTB, corte. **La interacción es la suma lineal conservadora; sin ablandamiento HAZ automático** |
| Madera — NCh1198 (tensiones admisibles) | ◐ | Flexión, corte, tracción, compresión (Ylinen), combinada. **Sin uniones ni compresión perpendicular a la fibra** |
| Sísmico — diseño por capacidad | ◐ | **Solo columna-fuerte/viga-débil** — sin amplificación por sobrerresistencia, detallamiento dúctil ni verificación de nudos/uniones |
| Servicio — deflexión y deriva de entrepiso | ✔ | Deflexión L/límite; deriva NCh433 0.002 · ASCE 7 0.02 · EC8 0.01 (compara valores entregados) |
| Auto-diseño — selección de perfil desde un catálogo | ✔ | Elige el perfil más liviano que cumple; **no es un optimizador continuo/de costo** |
| Diseño de uniones, detallamiento de armadura, verificación de torsión de barra | ✘ | Fuera de alcance |

### Interoperabilidad, reportes y automatización

| Capacidad | | Notas |
| --- | :-: | --- |
| `.s3d` nativo (JSON) — round-trip de fidelidad total | ✔ | Modelo + ajustes de diseño; el contenedor de ida y vuelta |
| Importar/exportar — SAP2000, ETABS, SOFiSTiK, Abaqus/CalculiX, OpenSees, NODEX, VECTOR | ◐ | **Geometría + cargas, nunca resultados.** Varios exportadores descartan cargas distribuidas y/o liberaciones (cada uno avisa) |
| IFC (BIM) | ◐ | **Solo geometría en ambos sentidos**; la exportación pierde Iy y J; sin cargas/apoyos/resultados |
| DXF / vector CAD | ✘ | Sin intercambio DXF |
| Reportes — PDF imprimible, Word `.docx`, SVG de espectro | ✔ | Portada, bases de diseño, cargas, imágenes, participación, D/C, derivas |
| Exportación CSV — resultados estáticos/modales/espectrales, matrices K/M de elemento y globales, historia en el tiempo | ✔ | |
| API pública headless (`js/api/portico.js`) | ✔ | Construir modelo, correr todos los solvers, post-proceso, diseño y servicio — scriptable en Node o el navegador, sin DOM |
| Extensibilidad — registrar análisis, códigos de diseño y formatos IO propios | ✔ | `registerAnalysis` · `registerDesignCode` · `registerFormat` |

Donde una fila es ◐ o ✘, la razón es una decisión deliberada y acotada — detallada en
[§8 Alcance y limitaciones](#8-alcance-y-limitaciones) y, para los números, validada en el
[Manual de Verificación](verification-manual.es.md).
...
- **El espectro de respuesta corre una dirección horizontal (X o Y) por llamada** — no hay dirección
  vertical (Z) ni combinación direccional automática (100/30, casos de signo ±).
- **La interacción P–M de hormigón es solo para secciones rectangulares**; no hay detallamiento de
  armadura (anclaje, empalmes, espaciamiento, recubrimientos mínimos), no hay verificación de torsión de
  barra (la demanda T se recoge pero no se verifica) y **no hay diseño de uniones** en ningún material.
- La **base de perfiles es solo europea** (IPE / HEA / HEB, más CHS / RHS) — no se incluyen tablas
  americanas W (AISC), aunque cualquier sección puede ingresarse por sus propiedades numéricas.
- **Las cargas estáticas son nodales, distribuidas o térmicas** — no hay tipo de carga puntual en barra
  (pon un nodo en el punto, o malla) ni tipo de carga de presión/cara en áreas (la carga de área es peso
  propio o térmica).
- **La interoperabilidad es geometría + cargas, nunca resultados.** Los conversores de texto (SAP2000,
  ETABS, SOFiSTiK, Abaqus, OpenSees, NODEX, VECTOR) descartan cargas distribuidas y/o liberaciones en
  varios exportadores — cada uno emite una advertencia; IFC es solo geometría en ambos sentidos y pierde
  `Iy`/`J` al exportar. No hay intercambio DXF/CAD. Solo el `.s3d` nativo hace round-trip con fidelidad
  total.
