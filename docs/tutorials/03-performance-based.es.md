...
| Período efectivo `Te` (≈ modal elástico) | 0.40 s |
...
`SD1 = 0.9 g`. El período de esquina es `Ts = SD1 / SDS = 0.60 s`; como `Te = 0.40 s < Ts`, la
...
| `C1` | desplazamiento inelástico vs elástico | 1.23 |
| `C2` | estrangulamiento / degradación | 1.08 |
| — | razón de resistencia `R = Sa/(Vy/W)·Cm` | 4.2 |

El desplazamiento espectral elástico del SDOF es `Sa·g·(Te/2π)² = 1.5·9.81·(0.40/2π)² = 0.060 m`, así
que

```
δt = 1.40 · 1.23 · 1.08 · 0.060 = 0.111 m   (deriva de techo 0.63 %)
```

## Paso 4 — Punto de desempeño y nivel

Ubicando `δt = 111 mm` en la curva de capacidad — con los límites de deriva de techo del pórtico de
acero **IO ≈ 0.7 %**, **LS ≈ 2.5 %**, **CP ≈ 5 %** sombreados — el punto de desempeño cae **justo en el
codo de fluencia**, dentro de la banda **IO**:
...
*Figura 1. Punto de desempeño (δt = 111 mm) sobre la curva de capacidad, en la banda de Ocupación Inmediata.*

| Nivel de desempeño | Límite de deriva de techo | Desplazamiento | δt = 0.111 m |
| --- | --- | --- | --- |
| Ocupación Inmediata | 0.7 % | 0.12 m | **justo por debajo → cumple** |
...
El estado del pushover en `δt` muestra solo un **puñado de rótulas plásticas** (los primeros extremos de
viga, amarillo) — el pórtico apenas está en fluencia. Esa es la imagen física de la **Ocupación
Inmediata**: la estructura es esencialmente elástica bajo el sismo de diseño.

![Estado del edificio en el punto de desempeño — solo se formaron las primeras rótulas de viga (Ocupación Inmediata).](img/t3-02-performance-state.png)

*Figura 2. El pórtico en el punto de desempeño — fluencia incipiente (IO).*
...
- Para este pórtico, incluso ante un **sismo de diseño de alta sismicidad** (`SDS = 1.5 g`),
  `δt = 111 mm` (deriva de techo 0.63 %) cae en el **codo de fluencia → Ocupación Inmediata**, con
  amplia reserva hacia Seguridad de Vida (0.44 m) y Prevención de Colapso (0.82 m). El pórtico
  pilar-fuerte/viga-débil es holgadamente seguro.
